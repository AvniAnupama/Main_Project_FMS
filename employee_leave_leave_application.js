...
var employee= $('select[name=employee]').find(':selected').text();
var leave_type= $('select[name=leave_type]').find(':selected').text();
var leave_reason= formData.get("leave_reason");
...
var end_date= formData.get("end_date");
                $.ajax({
                    ...
                    success: function (res) {
						...
						var Tdelete='<a href="#" onclick="UserDelete('+index+','+InIndexIncrement+');" data-toggle="tooltip" data-placement="top" title="" data-original-title="Delete"><button class="btn btn-danger">Delete</button></a>';
						table.row.add([AddIndexIncrement,employee,leave_type,leave_reason,start_date,end_date,edit,Tdelete]).draw();
						}
						else{
...
function UserDelete(val,index) {
		...
		type: "POST",
		url: FormAction,
		data:{'employee_leave_leave_application_id':val},
		success: function(res){
			if(res == "1"){
